...
(function () {
  ...
  function ensureNav() {
    ...
    nav.innerHTML =
      ...
      '<a href="/kalkulyatory.html">Калькуляторы</a>' +
  ...
  function injectBreadcrumbJsonLd() {
    ...
    el.querySelectorAll('a').forEach(function (a) {
      ...
      if (href.indexOf('http') !== 0) {
        if (href === '/' || href === '') url = 'https://spk-effa.ru/';
        else if (href.charAt(0) === '/') url = 'https://spk-effa.ru' + href;
        else {
          try { url = new URL(href, window.location.href).href; } catch (e) { url = window.location.href; }
        }
      }
    ...
    if (parts.length && items.length && parts[parts.length - 1] !== items[items.length - 1].name) {
      items.push({ '@type': 'ListItem', position: position, name: parts[parts.length - 1], item: window.location.href.split('#')[0] });
    }
  ...
  function ensureFormConsent() {
    document.querySelectorAll('form').forEach(function (form) {
      ...
      if (note) note.innerHTML = 'Подробнее — в <a href="/privacy.html">политике конфиденциальности</a>';
    });
  }
